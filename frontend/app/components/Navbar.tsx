...
export default function Navbar({ showBackButton = false }: NavbarProps) {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-orange-100 bg-white/85 backdrop-blur">
      <div className="max-w-6xl mx-auto flex items-center justify-between px-4 sm:px-6 py-3 gap-2">
        {/* Left: Logo or Back Button */}
        {showBackButton ? (
          <button
            onClick={() => router.back()}
            className="inline-flex items-center gap-2 px-3 py-2 rounded-xl bg-white border border-slate-200 shadow-sm text-slate-700 font-medium hover:bg-slate-50 hover:shadow-md hover:border-slate-300 active:scale-95 transition-all flex-shrink-0"
          >
          ...
          <Link href="/" className="flex items-center gap-2 min-w-0">
            <div className="h-9 w-9 rounded-xl bg-gradient-to-tr from-orange-400 to-amber-300 shadow-md flex items-center justify-center font-bold text-white flex-shrink-0">
              N
            </div>
            <div className="leading-tight min-w-0">
              <div className="font-semibold text-base md:text-lg truncate">Novel Recommender</div>
        ...
        <div className="flex items-center gap-2 md:gap-4 flex-shrink-0">
          ...
          <button
            ...
            className="md:hidden p-2 rounded-xl text-slate-700 hover:bg-orange-50 hover:text-orange-600 transition-colors flex-shrink-0"
            aria-label="เปิดเมนู"
          >
          ...
          {!user && (
            <>
              <Link
                href="/auth/login"
                className="px-3 py-1 md:px-4 md:py-1.5 rounded-full border border-orange-300 text-xs md:text-sm font-medium text-orange-600 bg-orange-50 hover:bg-orange-100 transition-colors flex-shrink-0"
              >
                เข้าสู่ระบบ
              </Link>
              <Link
                href="/auth/register"
                className="hidden sm:inline-block px-4 py-1.5 rounded-full bg-orange-500 text-sm font-medium text-white hover:bg-orange-600 transition-colors flex-shrink-0"
              >
                ลงทะเบียน
              </Link>
            </>
